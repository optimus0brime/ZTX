...
int wipe_device(const char *dev){
    ...
    // Pass 1 = Zero fill, Pass 2 = AES-CTR random
    const char *src[2] = {
        "dd if=/dev/zero",
        "openssl enc -aes-256-ctr -pbkdf2 -pass pass:$(head -c128 /dev/urandom | base64) -nosalt </dev/zero"
    };

    long size = dev_size_bytes(dev); // True device size in bytes
    if (size <= 0) size = 1;

    for(int p=0; p<2; p++){
        char cmd[1024], buf[512];
        if(p==0)
            snprintf(cmd,sizeof(cmd),"%s of=%s bs=16M oflag=direct status=progress 2>&1", src[0], dev);
        else
            snprintf(cmd,sizeof(cmd),"%s | dd of=%s bs=16M oflag=direct status=progress 2>&1", src[1], dev);

        FILE *fp = popen(cmd,"r");
        if(!fp) return 0;

        long copied=0, last_copied=0;
        time_t start=time(NULL); int ch;

        while(fgets(buf,sizeof(buf),fp)){
            // Only parse lines with "bytes"
            if(strstr(buf,"bytes")){
                long val=0;
            ...
            double frac_pass = (double)copied / size;
            if(frac_pass > 1) frac_pass = 1.0;

            // Global progress across 2 passes
            double frac_global = (p + frac_pass) / 2.0;

            // Speed & ETA calculations
            time_t now = time(NULL);
            double elapsed = difftime(now,start);
            double speed = (elapsed > 0) ? (copied/1048576.0)/elapsed : 0.0; // MiB/s
            double remaining = (2.0*size) - ((p*size)+copied);
            double eta = (speed > 0) ? (remaining/1048576.0)/speed : 0.0;

            // Update ncurses UI
            werase(win); box(win,0,0);
            mvwprintw(win,0,2," Wiping Device ");
            mvwprintw(win,1,2,"Device: %s", dev);
            mvwprintw(win,2,2,"Pass %d/2: %s", p+1, (p==0)?"Zeroes":"AES-CTR Random");

            progress_bar(win,5,2,frac_global,w-10);

            mvwprintw(win,7,2,"Copied this pass: %.2f / %.2f GiB",
                      copied/1073741824.0, size/1073741824.0);
            mvwprintw(win,8,2,"Overall Progress: %.1f%%", frac_global*100);
            mvwprintw(win,10,2,"Speed: %.1f MiB/s", speed);
            mvwprintw(win,11,2,"ETA:   %.0f sec", eta);
            mvwprintw(win,13,2,"Q/ESC=Abort");
            wrefresh(win);
...
int handle_ssd(const char *dev){
    ...
    mvprintw(3,2,"Choose Wipe Method");
    mvprintw(5,2,"Enter = Controller Secure Erase (fast&secure)");
    mvprintw(6,2,"N = NVMe SES selection (if NVMe drive)");
    mvprintw(7,2,"F = Fallback AES-CTR overwrite (slower, not guaranteed)");
    mvprintw(8,2,"Q/ESC = Cancel");
    refresh(); int ch=getch(); endwin();

    if(ch=='q'||ch=='Q'||ch==27) return 0;

    if(ch=='f'||ch=='F') return 2; // fallback dd

    if(strstr(dev,"nvme")) {
        initscr(); box(stdscr,0,0);
        mvprintw(1,2,"NVMe Secure Erase: Choose");
        mvprintw(3,2,"1 = Standard (SES=1)");
        mvprintw(4,2,"2 = Enhanced (SES=2, if supported)");
        refresh(); int sel=getch(); endwin();
        char cmd[512]; int ret;
        if(sel=='2') snprintf(cmd,sizeof(cmd),"nvme format %s --ses=2",dev);
        else snprintf(cmd,sizeof(cmd),"nvme format %s --ses=1",dev);
        ret=system(cmd); return (ret==0)?1:0;
    } else {
        // SATA SSD (hdparm)
        // Check frozen state
        char cmd[512]; snprintf(cmd,sizeof(cmd),"hdparm -I %s | grep frozen",dev);
        int frozen=system(cmd);
        if(frozen==0){
            printf("Drive might be frozen. Please suspend/resume system to unfreeze.\n");
            return 0;
        }
        // Create random password
        char *pwd="TempErasePwd";
        snprintf(cmd,sizeof(cmd),"hdparm --user-master u --security-set-pass %s %s && hdparm --user-master u --security-erase %s %s",pwd,dev,pwd,dev);
        int ret=system(cmd); return (ret==0)?1:0;
    }
}
...
int main(){
    if(geteuid()!=0){ initscr(); box(stdscr,0,0);
        mvprintw(2,2,"Root required! Run with sudo."); refresh(); getch(); endwin(); return 1; }
